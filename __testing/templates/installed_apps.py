...
# ps_script_path = ["reg query",  "HKCU\Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Compatibility
# Assistant\Store", "/s"] # HKEY_CURRENT_USER\Software\Microsoft\Windows
# NT\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store POWERSHELL_PATH =
# r"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"  # POWERSHELL EXE PATH

# class Utility:  # SHARED CLASS TO USE IN OUR PROJECT

# @staticmethod    # STATIC METHOD DEFINITION def run_powershell(script_path, *params):  # SCRIPT PATH = POWERSHELL
# SCRIPT PATH,  PARAM = POWERSHELL SCRIPT PARAMETERS ( IF ANY )

# commandline_options = [POWERSHELL_PATH, script_path]  # ADD POWERSHELL EXE AND EXECUTION POLICY TO COMMAND VARIABLE
# #commandline_options = [POWERSHELL_PATH, '-ExecutionPolicy', 'Unrestricted', script_path]  # ADD POWERSHELL EXE AND
# EXECUTION POLICY TO COMMAND VARIABLE for param in params:  # LOOP FOR EACH PARAMETER FROM ARRAY
